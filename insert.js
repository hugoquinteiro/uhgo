...
var insere = async function insertCustomer(cab, itens){
  ...
  console.log(sql)
  console.log(values)
  let ins = await client.query(sql, values);
  ...
  if (ins.rowCount===1){
    ...
    const res = await client.query(`SELECT max(id) as id FROM pedido WHERE usuario= '${cab.usuario}'`);
    ...
    if(res.rows[0].id){
     ...
     itens.forEach(async element => {
      //console.log(element)  
      values = [res.rows[0].id, element[0], element[3],element[2], element[4]]
      ins = await client.query(sql, values, (err, res)=>{
